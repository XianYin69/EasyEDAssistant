...
# 中断机制（执行中暂停，SKILL.md §11.5）
- 两类中断点：强制（不可逆/不可单方面定，必须暂停等用户答复）与
  条件（信息缺失会改变设计或超出已授权范围才中断）。
- 强制中断点：S0 决策点摊牌、S3 模块拆分与页数、S4 compose --replace
  清页、P3 板框/安装孔方案。
- 条件中断点：S2 位号修复超范围、S5 回读差异超预期、P6 布线前门
  blocked/fail、P8 多电源域铺铜策略、P10 DRC WARN 清单、文档任务
  确认。
- 不中断：已确认需求与授权、有唯一正确答案的 guardrail、已授权范围
  内增量执行、纯读取与验证。
- 中断触发格式：[CHECKPOINT <阶段> | <类型>] 背景(≤3句含证据) +
  决策点(选项+推荐默认+后果) + 影响范围 + 等待用户回复。
- 用户响应：选择→记入授权沿用不重复问；补充→更新不变量重评估；
  中止→停止不回滚(无undo)报告当前状态；超时/无答复→保持暂停态，
  不猜默认值落笔，不自动推进。
- 门禁关系：blocked→条件中断需用户修环境；fail→条件中断用户决定改
  设计还是放宽(放宽须授权)；pass→静默通过；截图stale+数据通过→
  不中断但报告标注。
