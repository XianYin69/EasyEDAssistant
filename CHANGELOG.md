...
- 新增坐标地基条款 `坐标与数据模型.md`「画前坐标校正」：正式绘图前用数字+直线在画布上标坐标刻度——EDA 画布以 `line`/`text` 临时标尺核原点/网格/y 轴/边界；字符画蓝图以带刻度边框核比例尺与行列；核完据标尺落位，**正式图开始前移除全部坐标标尺并保存校正数据** `./tmp/design/<工程>-calib-<sch|pcb|newpart>.json`（画布/原点/网格/轴向/偏移/标尺图元清单，只留痕不复用）。
- 接入三类绘制：`绘制原理图.md` 增 3.5 步（标尺核原点 5 raw/y-UP/页边与标题栏 keepout）、`PCB绘制.md` 增 3.5 步（标尺核板框原点 mil/网格锚/露铜边距）、`新建元件与替代方案.md` 步 1 增 ③ 画前坐标校正（元件图刻度边框核本体外接矩形 + EDA 试放页临时标尺）；`部分绘制与截图.md` 与 `部分绘制与验收.md` 增「校正已完成」前置判据，错位先回 3.5 步重校。
- `PCB布局图.md` 判据、目录契约 `./tmp/design/` 行（增 calib json）、`SKILL.md §2` 新增「画前坐标校正（通用）」钉定行并版本对齐 0.12.36。
- **校验**：check-links 悬空 0/孤立 0；触及 .md ≤50 行。版本对齐 **0.12.36**。
...
- `references/元件检查/新建元件与替代方案/` 建立新元件节新增 **步 0「建库前摊技术细节（P0）」**（引脚功能/电气类型依据、符号划分思路、封装尺寸命名，逐条带手册页码）与 **步 1「独立元件图 + 引脚说明」**：落 `./tmp/newpart/<型号>-newpart.md`（引脚说明表 + box-drawing 字符画原理图符号 + 决策头），用户认可并 `chain-log add memory --by user` 前禁止建库；原建立步骤顺延。
- **三图独立**入册：元件图 `./tmp/newpart/` ↔ 原理图布局 `./tmp/plan/<工程>-sch-layout.md` ↔ PCB 布局 `./tmp/design/<工程>-pcb-layout.md` 三份各管各的，互不复制覆盖——`PCB布局图.md`、`布局计算纪律.md` 第 7 条、`SKILL.md` §2 新增「三图独立」钉定行同步。
- 接线：`元件检查.md` 判据、`器件库与块.md` 库无件出口、`嘉立创EDA指令索引.md` 新建元件行、`嘉立创元器件库为准.md` 第 4 条、`约束部分.md` 第 16 项摘要均补「先摊细节+出独立元件图拍板」前置；目录契约与 `文件与文件夹创建范围.md` 白名单登记 `newpart/`。
- **校验**：check-links 悬空 0/孤立 0；触及 .md ≤50 行；双链辩论通过；版本对齐 **0.12.34**。

### v0.12.33（2026-09-21）

**PCB 布局独立成图＋绘制前再次问布局＋库无件强制新建（嘉立创库为准延伸）**（用户指令 2026-09-21）：

- 新增 [`references/PCB绘制/PCB布局图/`](references/PCB绘制/PCB布局图/PCB布局图.md)（正本）：PCB 绘制前生成**独立字符画布局图** `./tmp/design/<工程>-pcb-layout.md`（box-drawing 板框/分区/位号/比例尺，依 pcb-spec+layout-intent+layout-calc 生成，**禁止复制原理图 `-sch-layout.md`**），**再次向用户询问布局**并拍板（决策头 + `chain-log add memory --by user`），未拍板禁止落位（P0）；布局偏离走「变更记录」只增不改。
- 布局文件两份分开：`PCB绘制.md` 第 3 步改写、`PCB制作.md` 第 2 步、`用户PCB配置文件.md` 新增「PCB 字符画布局图（第二道门）」门禁、`部分绘制与验收.md` 落位按图 + 布局同步回写、`布局计算纪律.md` 新增第 7 条、`SKILL.md` §2 放置钉定行同步；目录契约与 `lib/verification-delivery.md` §8.4、`lib/project-docs.md` §7.4（文件契约路径 + 再问布局）对齐。
- 库无件→依技术文档新建：`元件检查.md` 新增「库无件 → 依技术文档新建」判据；`封装与引脚检查.md` 判定分支补「库内无该器件」出口；`新建元件与替代方案.md` 命令行建立前置「先证库确无此件」+ 判据「新元件几何唯一依据技术文档」；`器件库与块.md` 新增「库无件出口」条目；`嘉立创元器件库为准.md` 第 4 条同步「库中确无该器件」触发条件。
- **校验**：check-links 悬空 0/孤立 0；触及 .md 均 ≤50 行；`chain-log.py verify` 无缺要素。版本对齐 **0.12.33**。

### v0.12.32（2026-09-21）

**三项新增：tmp 双链建立与维护、步骤边界自动压缩、嘉立创元器件库硬门禁**（用户指令 2026-09-21）：
...
**排布计算前必须先问用户排布意图**（用户指令 2026-09-19）：

- [`references/约束部分/布局计算纪律/`](references/约束部分/布局计算纪律/布局计算纪律.md) 新增第 1 条「先问排布意图」：跑 layout-calc 或进入任何放置/分区计算前，必须一次摊清——功能分区与大小、固定件/手摆件、信号流方向、热与 RF 要求、阵列/对称偏好、参考板；答复落盘 `./tmp/design/<工程>-layout-intent.md`，未答复不得开始计算与落位（P0 中断，禁代填默认）；原理图侧同等生效；原 1–5 条顺延为 2–6 条，违规后果同步改号。
- `SKILL.md` §2 放置钉定行插入「必须先问排布意图→落盘 intent→再算」；`用户PCB配置文件.md` 门禁「布局基线」补问意图前置；`约束部分.md` 第 15 项摘要同步；`layout-calc.py` docstring 定位句同步（无行为改动）。
- **校验**：check-links 悬空 0/孤立 0；所有触及 .md ≤50 行；layout-calc 冒烟复跑通过。版本对齐 **0.12.31**。

### v0.12.30（2026-09-19）

**布局以用户需求为主＋边距/位置计算器**（修正 v0.12.29「禁一切自动命令」过严）：

- 新增 `scripts/layout-calc.py`：PCB/原理图边距与位置计算器（`pcb gap/edge/zone`、`sch gap/edge`、`angle check`）；三行留痕，`--profile/--margin/--allow` 可覆盖默认保守档。
- 新增约束 [`references/约束部分/布局计算纪律/`](references/约束部分/布局计算纪律/布局计算纪律.md)（约束部分第 15 项）：先配置后放置、先算后摆、自动规整工具有边界——align/distribute/auto-place 是需求已定后的规整工具，不得无依据重排；手摆＝P0 immovable 先锁定快照。
- `SKILL.md` 放置钉定行改写：配置文件→layout-calc→按意图落位→规整工具；四步缺一不可。`干涉布线.md` step7 改「45° 规整，直角禁（R1 圆弧档除外）」；`用户PCB配置文件.md` 门禁新增布局基线条目。
- **校验**：check-links 198 md 悬空 0/孤立 0；layout-calc.py 各子命令冒烟通过。版本对齐 **0.12.30**。
...
- **新建 `references/运维与排障/`（2 文件，均 ≤50 行）**：①`常见错误速查.md`（四分类速查表：链路/流程/产物/纠错总则，症状→原因→解决三列，覆盖 link-probe 链路不通、cwd 守卫拒绝、visual-qa 截图硬门死锁、compat 接口漂移、版本钉定 mismatch 等高频故障）；②`低模型优化指南.md`（核心原则 5 条 + 常见低级错误对照表 + 建议执行节奏 + 低模型友好入口索引，降低参数量较小模型的出错率）。
- **约束正本扩展**：`约束部分.md` 总则新增第 7 条「故障排查路径」（≤50 行约束内追加）；`SKILL.md` §5 新增故障排查指针行（SKILL.md 44 行，仍 ≤50）。
- **脚本错误提示增强**：`link-probe.py` 链路全断时补三步排查指引（确认 EDA 启动→检查插件→启动 daemon）；`pcb-gate.py`/`sch-verify.py` 聚合输出逐子项补 "rc=N（失败项，查 stderr 或 JSON 中的 cmd/text 字段定位原因）" 字样，方便低模型快速定位根因。
- **校验**：19 py 全编译；`check-links` 197 md 悬空 0/孤立 0；所有触及文件 ≤50 行。版本对齐 **0.12.28**。
...
- **新正本 `references/处理设计问题/分歧修复稿/分歧修复稿.md`（32 行）**：触发判据（承诺 vs 实际不符＝分歧；设计错误/环境问题各归主线与桥接排查）；四件套规格——目录 `./tmp/divergence/NN-短标题/`：`00-错误说明.md`（命令/stdout/stderr 逐字引用+环境上下文引用 link-probe/compat/快照 tag）、`10-分歧记录.md`（skill 侧逐字引用文件+条款号 vs 实际行为+影响面）、`20-修复提示词.md`（维护窗口可直接粘贴的**提案**：目标文件精确路径+逐字替换文本+验证方式，开头声明生效唯一通道＝用户交维护会话按 RULE_EDIT 执行）、可选 `30-repair-<名>.py`（本会话临时绕行脚本，只住 tmp、头部标注替代对象与失效条件，禁止复制进 skill）。纪律：不替代用户摊牌主线、账目 `art:` 挂目录、未闭环不得记 pass、新会话读 tmp 先复述、往 skill 写一字仍 L3。
- **边界正本化**：`运行环境不可变` 新增 §五（14–15 条，47 行 ≤50）——tmp 草稿是「修复方向」的唯一允许写物，skill 只读不变；`SKILL.md` §1.2 补「唯一例外方向」一句。
- **闭环挂接（6 处）**：`处理设计问题.md` 流程首步改「先分类」+ 分歧分支 + 账目 art 追加（26 行）；`文件与文件夹创建范围` tmp 白名单加 `divergence/` + 工作区根 `<工程>-divergence-archive/` 移交目录；`目录契约` 表加 divergence 行；`文件命名规范` 工件式补 `NN-短标题/00|10|20|30-*` 式样；`清理.md` 顺序插入「3. 先移交分歧稿」（归档不参与 tmp 删除，防 v0.12.16 式账目自毁同类问题）；`交付报告.md` 新增字段 8「分歧与修复稿移交」（无分歧也须写「无分歧记录」）。
- 校验：`check-links` 195 md 悬空 0/孤立 0；全部触及文件 ≤50 行；分歧目录移交链（divergence→archive→报告字段8）三处互引一致。版本对齐 **0.12.26**。

### v0.12.25（2026-09-19）

**新增三件计算器（科学/电气/设计，用户初指快照目录、后更正为根 `scripts/`）+ 新约束「计算纪律」（第 14 项）接入流程**：

- **`scripts/calc-scientific.py`（科学计算器）**：AST 白名单求值（无 eval 注入）、SI 工程前缀（`2k*10u`→0.02、`4k7` 工程写法）、复数阻抗直角↔极坐标（`abs(50+30j)`→58.31∠30.96°）、单位换算（mil/mm/inch/F 前缀/℃°F）、常量（π/e/c/q_e/k_B/ε0/µ0）；纯 stdout，退出码 0/2。
- **`scripts/calc-electrical.py`（电气计算器）**：ohm（V/I/R/P 任二）、divider/led/rc（E24 就近标称内置）、**ipc2152**（I=k·ΔT^0.44·A^0.725，外 0.048/内 0.024，1oz=1.37mil——2A@10°C 外层→30.9mil 与经典曲线一致）、via-current（镀壁截面近似 12/0.8mil→1.56A 保守档）、**impedance**（Hammerstad 微带/带状线，明示 ±10% 与板厂工具复核义务）、l4、db；每项输出**「公式/代入/结果」三行**便于账目留痕。
- **`scripts/calc-design.py`（设计计算器）**：ldo-heat（θJA 需求 0.68W→≤103°C/W）、buck-l/buck-cout（12→3.3V/500kHz/30%→15.95µH+标准感建议）、battery、crystal-cl（2×(10−3)→E24 15p）、resistor（串并联 + worst/RSS 容差传播，并联敏感度 (R/Rᵢ)²·δRᵢ 数学修正）、copper-heat（0.05W/cm²/°C 经验系数，标注须实测复核）。
- **新约束正本 `references/约束部分/计算纪律/计算纪律.md`（35 行，第 14 项）**：凡进方案/参数表/图纸/账本的数值必先演算、三行输出整行复制进 `cmd:` 留痕、无记录＝未核实；IPC/阻抗/热近似与 `间距与线宽.md`/板厂参数**取严**；**不替代 route-check 几何门**（载流档答"多宽载多流"，不答"撞不撞"）；经验系数须原样转述保守性。三脚本零写盘零联网、stdout/stderr 强制 UTF-8（修复 Windows GBK 控制台 `−`/`²` 崩溃，与 v0.12.17 同类问题的脚本侧预防）。
- **流程接入（5 处）**：`约束部分.md` 第 14 条；`电气检查.md` 头部；`检查方案及敲定.md` 头部（数值先演算再进检）；`干涉布线.md` 第 4 条（载流/阻抗档计算留痕）；`干涉路径.md` 第 6 条（去耦/分压/晶振参数）。SKILL §4 一键清单 + README（树 3 行、十四项×2、设计执行段）同步。
- **过程修正（如实记录）**：初版三脚本按用户最初指定放 `Sample/easyeda-agent-connector/scripts/`，与快照只读/manifest 纪律冲突，用户更正「是项目根目录下的 scripts/」后已移出——快照恢复 39 文件全匹配（`eext-src verify` ok:true 复验），无需任何本地扩展豁免；`Set-Content -Encoding UTF8` 引入的 BOM 已剥除。
- 校验：三计算器 13 个子命令/用法功能实测值核正确；18 py 全编译；`check-links` 悬空 0/孤立 0；新文件 ≤50 行。版本对齐 **0.12.25**。

### v0.12.24（2026-09-19）

**原理图走线同步弃用迷宫/盲画，路径绘制逻辑与 v0.12.23 PCB 方法同构（用户指令：「原理图绘制也不能使用迷宫算法，而且新的路径绘制逻辑与刚刚PCB修改的逻辑相同」）**：

- **方法正本新建 `references/绘制原理图/干涉路径/干涉路径.md`**（34 行，与 `PCB绘制/干涉布线/` 同构四要素）：①网络标签-端口——连接链 pin→短桩 wire→netflag/netport，wire 的 net 由共线合并派生、端口名即网络标签，真值对账走 `sch connectivity`/跨页 `sch nets`；②向量-节点——手改路径逐段两点向量（全水平/竖直、落格、端点必须落在 pin 桩末端/端口/既有 wire 端点上，禁悬空节点）；③干涉演算——画前由 autoconnect 确定性规划器（"no safe candidate = 硬拒不落笔"，**非迷宫档，保留为短桩首选**）与格子几何推演承担，画后**即查** `sch bridge-check`（共线合并短路+孤儿桩，wire-tree 粒度＝原理图的干涉判官）+ `sch check`（叠加 marker）+ `layout-lint` + 标题栏 keepout，fail 即 `prim-delete` 精删重推、不带错续画；④硬编码档位——短桩方向长度档（power 上 20–40/ground 下 10–40/netport 外 20–60）与线宽档（信号1/电源2/总线3）引 输入读取.md 正本。执行序：block-apply/compose 保护队列 → autoconnect --spec 批量 → 剩余段逐向量；冲突解不开=改端口/摊牌，**不开盲画模式**。sch 侧当前无上游迷宫命令——禁令按「未来出现即禁」预置。
- **挂接改写（6 处）**：`绘制原理图.md` 第 4 步与连接表挂方法正本；`部分绘制与截图.md` 绘制步骤注入走线纪律与禁令；`连线与网络.md` 增 `sch wire` 行（ESCAPE HATCH **转正经逐段向量通道**、禁整串盲甩、落笔即查）+ 头部挂正本；`原理图指令.md` 索引行；`处罚机制` L3 补「原理图盲画走线（不经演算甩多点线 / 画后不即查 / sch 侧迷宫）」并修复本次编辑误吞的 daemon 条款；`SKILL.md` §2 方法钉定行扩为「原理图与 PCB 同一套」；README 设计执行段双侧化。
- 校验：`check-links` 悬空 0/孤立 0；新正本与全部改动文件 ≤50 行；引用命令（autoconnect/wire/bridge-check/connect/disconnect/netflag/nets/prim-delete/layout-lint）逐条对照本机 `sch --help` 与既有实测记录，无臆造。版本对齐 **0.12.24**。

### v0.12.23（2026-09-19）

**弃用上游迷宫自动布线 → 干涉演算+网络标签-端口+向量-节点布线法；丝印支持用户三类标注（板型号/版权/功能指引）（用户指令）**：

- **方法替换（正本新建 `references/PCB绘制/干涉布线/干涉布线.md`，40 行）**：`pcb autoroute`/`export-dsn`/`import-autoroute`（Freerouting 迷宫三件套）**禁调用**（点名要求须先复述禁令转用户书面拍板）；执行序钉死：端口=焊盘（`pcb list --include-pads` + `pcb nets`，落笔必带 `--net` 网络标签绑定）→ 档位=硬编码 `间距与线宽.md` 表（`net-classes` 回读核对）→ `route-critical`（P7.0 关键网）→ `route-short`（per-net MST 障碍感知档，实测 help 自证 "NOT a maze router"，允许）→ 剩余长网**人工向量分解**（grid-snap 落格、45°/90°、via-hop 换层）→ 每段先过干涉演算再 `pcb track`。让位链：绕距→批准换层→`rip-up --net` 局部让位→死结摊牌，**任何情况不回开迷宫**。上游「长跳线留给 maze tier」惯例声明为不适用。
- **新增干涉演算一键脚本 `scripts/route-check.py`（15 个 py 之一）**：候选向量段 vs 异网 track/via/焊盘/keepout region/板边 keep-in 的几何距离判定（线段-线段/点-段/段-矩形，同网豁免；角色默认档内置与硬编码表同源，--width/--clearance 可覆盖）；报告落 `./tmp/pcb/routecheck-*.json`；**退出码 0 才准落笔，1=干涉列表，3=回读失败（演算未成立≠通过，fail-closed）**；键形宽容抽取（信封剥壳、多候选键）。几何内核 3 断言 + 离线窗口场景活体 rc=3 fail-closed 实测。
- **丝印能力（正本新建 `references/PCB绘制/丝印标注/丝印标注.md`，31 行）**：三类用户丝印——板型号（型号/版本/日期码/序列号区/厂标，板角必含）、版权（逐字文本+可选 logo SVG，**Agent 禁代拟授权内容**）、功能指引（`silk-zone-outline` 分区框 + `silk-netnames` + `silk-label-pads`）；执行序=收集拍板（中断点）→分区框→逐条 `silk-add` 落位回读→合规自检（阻焊丝印与拼板.md 字号/线宽/上盘禁令）→逐条截图用户核对；布线终检后执行。**7 条 silk 命令全部本机 `--help` 实测存在**，无臆造。
- **配套改写（8 处）**：`布线与过孔.md` 三迷宫命令标【已弃用】+ 布线纪律重写 + route-check 条目；`PCB指令.md` 索引行；`部分绘制与验收.md` 第 3 步布线挂方法正本、新增第 4 步丝印；`PCB绘制.md` 总索引第 4 步 + 连接表两行；`PCB设计规范.md` 第 5 条去「稠密板走 autoroute」；`用户PCB配置文件.md` **R2 从「是否允许自动布线」改为「向量布线许可面」**（route-short 白名单/via-hop 批准/rip-up 授权范围）+ **R4 从「版权与标识」扩展为「丝印三清单」**；`GUI指令映射` 自动布线行标弃用；`处罚机制` L3 补「调用迷宫三件套 / 绕过 route-check 落笔」。
- **SKILL.md** §2 加方法钉定声明行、§4 一键脚本清单补 route-check；README 脚本树 + 设计执行段同步。
- 校验：15 py 全编译过；route-check 几何单测 3/3、fail-closed 活体实测；`check-links` 悬空 0/孤立 0；全部 references（非 lib）≤50 行（GUI映射恰 50）；干涉布线/丝印标注内引用的 CLI 与文件逐条对照真机/磁盘。版本对齐 **0.12.23**。
