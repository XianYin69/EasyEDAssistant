...
python <SKILL_DIR>/scripts/visual-qa.py --project <name> --doc <page-uuid> --both \
    --artifacts-dir ./tmp/snapshots

# 原理图逐件纠错（绘制步每落位一个器件调用一次；截图 + 图纸边界越界核验）
python <SKILL_DIR>/scripts/visual-qa.py --project <name> --doc <page-uuid> --schematic \
    --artifacts-dir ./tmp/snapshots

# 仅 PCB，strict（WARN 也判阻塞；--pcb-doc 供 stale 时自动切前台重试）
python <SKILL_DIR>/scripts/visual-qa.py --project <name> --pcb-doc <pcb-uuid> --pcb --strict \
    --artifacts-dir ./tmp/snapshots

# 跳过截图采集（只用已有数据打分；不触发 canvas-freeze）
python <SKILL_DIR>/scripts/visual-qa.py --project <name> --pcb --no-snapshot
```
