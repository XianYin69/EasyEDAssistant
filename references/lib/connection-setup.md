...
- 开始工作前按以下优先级检测：
  1. 先尝试 `7655` 端点（MCP HTTP）；连通再探测 `60832`。
  2. **两条都连通 → 优先全程使用 CLI/daemon**：写/批量操作一次命令一次派发，
     往返与 token 开销低于逐次 MCP 调用；MCP 适合交互式细查。
  3. 仅 MCP 连通 → 全程使用 MCP；仅 daemon 连通 → 全程使用 CLI/daemon。
- **一旦选定链路即全程使用**，不重试另一条链路，不做"莫名奇妙的检测"。
