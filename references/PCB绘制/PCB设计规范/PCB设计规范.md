...
- **官方入口**：[`../../官方文档映射/官方文档映射.md`](../../官方文档映射/官方文档映射.md)（菜单项 ↔ CLI 对照；官方只说明「在哪设」，数值以 daemon 规则代码与 JLC 官网为真值）。
- **运行时可读**：`pcb drc-rules`（live 规则）、`pcb layers`（层数与内层类型）、`pcb stackup show`（叠层）、`pcb net-classes`（线宽阶梯）。
